...
and toec_instr asmOp env i =
    ...
    | Ccall (lvs, f, es) ->
        ...
        let i = List.fold (fun acc pre -> toec_assert env pre @ acc ) [] pre in
        let env = List.fold_left add_var env tmps in
        let i = i @
                (ec_leaks_es env es) @
                (ec_pcall env lvs2 otys [get_funname env f] args)
        in
        let i = List.fold_left (fun acc post -> toec_assume env post @ acc ) i post in
        List.fold_left2
          (fun acc lv e ->
             let e = toec_cast env (ty_lval lv) e in
